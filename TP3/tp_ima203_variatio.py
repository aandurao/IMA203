...
def viewimage(im,normalise=True,MINI=0.0, MAXI=255.0,titre=''):
    ...
    if platform.system()=='Darwin': #on est sous mac
        ...
        prephrase='gimp-2.10 '#Remplacer D:/... par le chemin de votre GIMP
        endphrase= ''
    else:
...
b=100
im=imread('lena.tif')
#imb = im + np.random.normal(0,5, im.shape).reshape(im.shape)
imb = degrade_image(im, 25)
d1 = norm2(im-imb)**2
eps=0.01
i = 0
def f(x):
    return norm2(minimisation_quadratique(imb,x)-imb)**2 - d1
while b-a>eps:
    m = (a+b)/2
    if(f(a)*f(m)<=0):
        b=m
...
##
im=imread('lena.tif')
imb = im + np.random.normal(0,25, im.shape).reshape(im.shape)
def f(x):
    return np.linalg.norm(minimisation_quadratique(imb,x)-im)**2
...
imb = im + np.random.normal(0,5, im.shape).reshape(im.shape)
def f(x):
    return np.abs(minimisation_quadratique(imb,x)-imb) - np.abs(im-imb)
...
im=imread('lena.tif')
imb = im + np.random.normal(0,5, im.shape).reshape(im.shape)
energy = minimise_TV_gradient(imb, 1, 0.9, 50)
n = np.arange(0, len(energy[1]), 1)
print(energy[1][-1])
plt.plot(n, energy[1])
plt.show()
...
print(E2_nonperiodique(vartotale_Chambolle(imb, 40), im, 40))
...
plt.plot(10**vk,errvt, c='g')
#plt.plot(10**vk,errq, c='r')
plt.show()
